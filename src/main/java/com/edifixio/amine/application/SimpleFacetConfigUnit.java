...
public class SimpleFacetConfigUnit extends JsonObjectConfig {
	...
	public void process(JsonObject aggQuery, Map<String, FacetableAggr> facetsData,
		...
		if (!aggQuery.has(facetName)) {
			System.out
					.println("Exception SimpleFacetConfigUnit :there are no facet named  :" + facetName + " in Query ");
			return;
		}
		...
		if (facetableAggr.isTermAggr()) {
			...
			String fieldAgg = jsFacetAgg.getAsJsonObject(SimpleFacetsConfig.TERMS).get(SimpleFacetsConfig.FIELD)
					.getAsString();
			/****************************/
			while (bucketsIter.hasNext()) {
				...
				// System.out.println("call SimpleFacetsConfig js :---->
				// "+jsFacetAgg+"\n"+AGGS);
				// System.out.println("----> call SimpleFacetsConfig facets
